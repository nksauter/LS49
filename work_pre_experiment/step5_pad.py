...
def run_sim2smv(prefix,crystal,spectra,rotation,rank,quick=False):
  ...
  print "_________"
  print str(GF.cb_op_C2_to_P)
  # now we are going to work up the unit cell and orientation
  from cctbx import crystal_orientation
  to_simtbx_Ori = crystal_orientation.crystal_orientation(Amatrix_rot, crystal_orientation.basis_type.direct)

  P1_ortho_Ori = crystal_orientation.crystal_orientation(sqr(sfall_main.unit_cell().orthogonalization_matrix()).transpose(),
                 crystal_orientation.basis_type.direct)

  to_simtbx_Ori.show(legend="to_simtbx_Ori")
  P1_ortho_Ori.show(legend="P1_ortho_Ori")

  C2_ortho_prime = P1_ortho_Ori.change_basis(GF.cb_op_C2_to_P.inverse())
  C2_ortho_prime.show(legend="C2_ortho_prime")

  C2_simtbx_prime = to_simtbx_Ori.change_basis(GF.cb_op_C2_to_P.inverse())
  C2_simtbx_prime.show(legend="C2_simtbx_prime")
