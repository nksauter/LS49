...
class lbfgs_fpfdp_fit:
  ...
  def lsq_target_function(self,title,debug):
    ...
    #sel0 = flex.size_t([p[0] for p in matches.pairs()])
    #sel1 = flex.size_t([p[1] for p in matches.pairs()])
    #selected_I = self.intensity.select(sel1)
    #selected_H = self.miller.select(sel1)
    #for ikey in xrange(len(selected_H)):
    #  per_HKL_I[selected_H[ikey]] = selected_I[ikey]
     # miller_lookup[selected_H[ikey]] = ikey
    ...
    # Evaluate the overall target function for the reduced state
    LSQ = 0.
    self.debug = flex.double()
    self.debug_d1 = flex.double()
    ...
    for key in self.G.images_strong:  # 733 images
      for HKL in self.G.images_strong[key]:  # should be 3414 terms visited in the double loop
        #from IPython import embed; embed()
        if debug=="Intensity":
          self.debug.append(per_HKL_I[HKL])
          self.debug_d1.append(self.d1[miller_lookup[HKL]])
  ...
  def functional(self,values):
    ...
    self.intensity0 = self.intensity
    f = self.lsq_target_function(title="Internal form",debug="Intensity")
    debug_I_naught = self.debug.deep_copy()
    debug_d1_naught = self.debug_d1

    v2 = [values[0]+0.0001, values[1], values[2], values[3]]
    self.miller, \
    self.intensity, \
    ...
    debug_I_one = self.debug

    print ("ASSERTION", len(debug_I_naught),len(debug_d1_naught),len(debug_I_one))
