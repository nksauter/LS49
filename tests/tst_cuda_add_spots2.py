...
from LS49.sim import step5_pad2

step5_pad2.add_spots_algorithm = "cuda"
...
def run_polychromatic(create):
  from LS49.sim.step5_pad2 import tst_all
  tst_all(quick=False,prefix="cuda_step5_pad2") #actually perform the 100-channel simulation
...
if __name__=="__main__":
  ...
  if mode == "laue":
    ...
    ls49_big_data = os.environ["LS49_BIG_DATA"] # get absolute path from environment
    compare_two_images(reference=os.path.join(ls49_big_data,"reference","cuda_step5_pad2poly_000000_truth.img.gz"),
                       test="./cuda_step5_pad2poly_000000.img.gz")
  else:
    run_monochromatic()
    ls49_big_data = os.environ["LS49_BIG_DATA"] # get absolute path from environment
