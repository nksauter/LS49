...
class MCMC_manager:
  def get_amplitudes(self, dials_model, refl_table, test_without_mpi=True):
    ...
    #plt.plot(range(MA.size()), MA.data())
    #plt.show()
    ...
    if 0:# except ImportError:
      self.gpu_channels_singleton = None
  ...
  def job_runner(self,i_exp=0,spectra={}):
    ...
    include_background = self.params.include_background   # default is to add water background 100 mm thick
