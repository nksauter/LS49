#!/bin/bash -f
# create a special version of the data on CFS, to test perlmutter performance
# while accessing data on spinning disk.  One-time use, not generally recommended.
export CFSX=${SCRATCH}/cfsx/cdirs/m3562/swissfel
export CFS_BERNINA=$CFSX/../nks/bernina
export ACTUAL_CFS=/global/cfs/cdirs/m3562/swissfel/
for run in {795..805}
do
    echo $run
    ...
    raw_file=${ACTUAL_CFS}/raw/Cyt/run_000${run}.JF07T32V01.h5 \
    ...
    output_file=${ACTUAL_CFS}/../nks/run_000${run}.JF07T32V01_temp_readout_master.h5 \
    raw=False \
    beam_file=${CFSX}/raw/Cyt/run_000${run}.BSREAD.h5

    libtbx.python ${MODULES}/cctbx_project/xfel/swissfel/sync_jf16m_geometry.py \
    nexus_master=${ACTUAL_CFS}/../nks/run_000${run}.JF07T32V01_temp_readout_master.h5 \
    new_geometry=${BERNINA}/split_0000.expt \
    output_file=${ACTUAL_CFS}/../nks/run_000${run}.JF07T32V01_readout_master.h5
done
