...
Changes in step6 relative to step5
Estimate the gain inline.  Should be 1.0
Necessary:  calibration noise = 0%, not 3%
Necessary:  readout noise = 0, not 3 ADU
"""
...
def run_sim2smv(prefix,crystal,spectra,rotation,rank,quick=False):
  ...
  SIM.detector_calibration_noise_pct = 0.0
...
def estimate_gain(raw_data,offset=0,algorithm="kabsch",kernel_size=(10,10), output_gain_map=None, max_images = 1):
  ...
  if True:
    ...
    print ("This small section",len(small_section),"mean is", flex.mean(small_section.as_double()))
    ...
    mask = [flex.bool(raw_data[i].accessor(), True)
                for i in range(len(raw_data))]
